...
if(isset($_POST['save'])){
  ...
  $query = $selectedConn->prepare("INSERT INTO tblstudents (firstName, lastName, otherName, admissionNumber, password, classId, classArmId, dateCreated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
  $password = '12345'; // Plain text password
  $query->bind_param("ssssssss", $firstName, $lastName, $otherName, $admissionNumber, $password, $classId, $classArmId, $dateCreated);
...
if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit") {
  ...
  if (isset($conn[$dbKey])) {
    ...
    if (isset($_POST['update'])) {
      ...
      $query = $conn[$dbKey]->prepare("UPDATE tblstudents SET firstName = ?, lastName = ?, otherName = ?, admissionNumber = ?, password = ?, classId = ?, classArmId = ? WHERE Id = ?");
      $password = '12345'; // Plain text password
      $query->bind_param("sssssssi", $firstName, $lastName, $otherName, $admissionNumber, $password, $classId, $classArmId, $Id);
