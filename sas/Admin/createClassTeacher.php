...
if (isset($_POST['save'])) {
  ...
  // Ensure classArmId is provided (use empty string when not supplied).
  ...
  // NOTE: tblclassteacher has a NOT NULL classArmId column in the DB, so include it here (use empty string if none).
  $stmt = $selectedConn->prepare("INSERT INTO tblclassteacher (firstName, lastName, emailAddress, password, phoneNo, classId, classArmId, dateCreated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
  // types: firstName(s), lastName(s), emailAddress(s), password(s), phoneNo(s), classId(i), classArmId(s), dateCreated(s)
  $stmt->bind_param("sssssiss", $firstName, $lastName, $emailAddress, $sampPass, $phoneNo, $classId, $classArmId, $dateCreated);
      if ($stmt->execute()) {
...
if (isset($_GET['Id']) && isset($_GET['action']) && $_GET['action'] == "edit") {
  ...
  if (isset($conn[$dbKey])) {
    ...
    if (isset($_POST['update'])) {
      ...
      $phoneNo = $_POST['phoneNo'];
      $classId = $_POST['classId'];
      // Ensure classArmId is captured when updating (use empty string if not supplied)
      $classArmId = isset($_POST['classArmId']) ? $_POST['classArmId'] : '';
      $dateCreated = date("Y-m-d");

      // Include classArmId in the update to keep table consistent (table requires classArmId)
      $stmt = $conn[$dbKey]->prepare("UPDATE tblclassteacher SET firstName = ?, lastName = ?, emailAddress = ?, phoneNo = ?, classId = ?, classArmId = ? WHERE Id = ?");
      $stmt->bind_param("ssssisi", $firstName, $lastName, $emailAddress, $phoneNo, $classId, $classArmId, $Id);
      if ($stmt->execute()) {
                  ...
                  <form method="post">
                    ...
                    <div class="form-group row mb-3">
                      <div class="col-xl-6">
                        ...
                        <!-- Hidden field for classArmId (DB requires this column) -->
                        <input type="hidden" name="classArmId" id="classArmId" value="">
                      </div>
                    </div>
                    <?php
                    if (isset($Id)) {
